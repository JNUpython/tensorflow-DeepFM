...
sys.path.append("..")  # 保证父目录的import
gini_scorer = make_scorer(gini_norm, greater_is_better=True, needs_proba=True)


def preprocess(df):
    cols = [c for c in df.columns if c not in ["id", "target"]]
    df["missing_feat"] = np.sum((df[cols] == -1).values, axis=1)  # 每个样本的缺失数据的数量
    # logger.info(df["missing_feat"].describe())
    df["ps_car_13_x_ps_reg_03"] = df["ps_car_13"] * df["ps_reg_03"]  # 创建一个新的特征
    return df
...
# folds
folds = list(
    StratifiedKFold(n_splits=config.NUM_SPLITS, shuffle=True, random_state=config.RANDOM_SEED)
        .split(X_train, y_train))

logger.info(len(folds))
logger.info(len(folds[0]))
logger.info(len(X_train))
logger.info(len(folds[0][0]))
logger.info(len(folds[0][1]))
